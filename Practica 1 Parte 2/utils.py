...
""" Ramificacion y acotacion Queue Ordena por path_cost al sacar """
class RamumEtVinctum(Queue):
  
    #Constructor. Contiene la lista
    def __init__(self):
        self.L = []
        self.start = 0
        self.contadorNodos = 0
    
    #Add un item a la lista
    def append(self, item):
        self.L.append(item)
    
    #Devuelve el length de la lista
    def __len__(self):
        return len(self.L) - self.start
    
    #Concatena una lista pasada como parametro a nuestra lista
    def extend(self, items):
        self.L.extend(items)
        self.contadorNodos = self.contadorNodos + 1
    
    #Sacamos el primer valor de la lista(pos. 0=self.start)
    def pop(self):
        #Antes ordenamos la lista de menor a mayor en orden de coste de la trayectoria
        self.L.sort(key=lambda aux: aux.path_cost)
        e = self.L[self.start]
        self.start += 1
        if self.start > 5 and self.start > len(self.L) / 2:
            self.L = self.L[self.start:]
            self.start = 0
        return e
""" FIN """

""" Ramificacion y acotacion Queue con estimacion Heuristica """
class RamumEtVinctumHeuristic(Queue):
    ...
    def pop(self):
        #Antes ordenamos la lista de menor a mayor en orden de coste de la trayectoria
        self.L.sort(key=lambda aux: aux.path_cost + self.problem.h(aux))
        e = self.L[self.start]
        self.start += 1
        if self.start > 5 and self.start > len(self.L) / 2:
            self.L = self.L[self.start:]
